...
module.exports = {
    Query: {
            ...
            if (context.req && context.req.headers.authorization) {
                ...
                if (decodedToken) {
                    try {
                        ...
                        const messageFromMe = await Message.find({ from: decodedToken.id }).sort({ createdAt: -1 })

                        users = users.map(otherUser => {
                            const latestMessage = messageFromMe.find(m => m.from === otherUser.id || m.to === otherUser.id)
        ...
        getUsersMessage: async (_,{ userId }, context) => {
            if (context.req && context.req.headers.authorization) {
                ...
                if (decodedToken) {
                    try {
                        const allMessageFromMe = await Message.find({ $or: [
                            {from: userId},
                            {to: userId}
                            ...
                            if (singleUsers.length <= 0) {
                                ...
                                for (let i = 0; i < singleUsers.length; i++) {
                                    if (singleUsers[i].username === user.username) {
                                        break
                                    }else if((singleUsers[i].username !== user.username) && (i === (singleUsers.length - 1))) {
                        ...
                        users = singleUsers.map(otherUser => {
                            const latestMessage = allMessageFromMe.find(m => m.from === otherUser.id || m.to === otherUser.id)
                            // console.log(latestMessage)
                            otherUser.latestMessage = latestMessage
                            return otherUser
                        })
    ...
    Mutation: {
        register: async (_, { registerInput: { username, password, confirmPassword, email} }) => {
